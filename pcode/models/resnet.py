...
class ResNetBase(nn.Module):
    def _decide_num_classes(self):
        if self.dataset in ["cifar10", "svhn", "fashionmnist", "mnist"]:
            return 10
...
class ResNet_cifar(ResNetBase):
    def __init__(
        ...
        self.conv1 = nn.Conv2d(
            in_channels=3 if "mnist" not in self.dataset else 1,
        ...
        self.avgpool = nn.AvgPool2d(kernel_size=8 if "mnist" not in self.dataset else 7)
        self.classifier = nn.Linear(
    ...
    def forward(self, x):
        ...
        x = self.layer3(x)
        activation3 = x
        x = self.avgpool(x)  # kernel=7 when image is [28x28]; kernel=8 when image is [32x32];
        x = x.view(x.size(0), -1)
...
class CustomResNet50(nn.Module):
    def __init__(self, num_classes=10):
        ...
        # Add a custom fully connected layer for the desired number of classes
...
class ModifiedResNet18(nn.Module):
    def __init__(self, dropout_prob=0.5, num_classes=10):
        ...
        # Extract the feature extraction layers (excluding the final fully connected layer)
        self.features = nn.Sequential(*list(resnet18.children())[:-1])

        # Add dropout to the feature extraction layers
        for layer in self.features:
            if isinstance(layer, nn.Conv2d):
                layer.add_module("dropout", nn.Dropout2d(p=dropout_prob))
...
def resnet(conf, arch=None):
    # if conf.data == "emnist":
    #     return CustomResNet50(num_classes=47)
    # else:
    #     return CustomResNet50(num_classes=10)

    # dropout
    # parts = conf.arch.split('_')
    # if len(parts) > 1:
    #     dropout_prob = parts[1]
    # else:
    #     dropout_prob = 0
    # return ModifiedResNet18(dropout_prob=dropout_prob)
