...
class DataPartitioner(object):
    ...
    def partition_indices(self, indices):
        ...
        if self.conf.data != "movie_reviews"and self.conf.graph.rank == 0:
            global shared_targets_of_partitions
            if self.conf.data == "svhn":
                shared_targets_of_partitions = record_class_distribution(
                    self.partitions, self.data.labels, print_fn=self.conf.logger.log
                ).items()
            else:
                shared_targets_of_partitions  = record_class_distribution(
    ...
    def _create_indices(self, indices):
        if self.partition_type == "origin":
            ...
            targets_labels = self.data.labels if self.conf.data == "svhn" else self.data.targets
...
def get_shared_targets_of_partitions():
    return shared_targets_of_partitions