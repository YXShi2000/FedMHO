...
class BaseKTSolver(object):
    ...
    def divergence(self, student_logits, teacher, use_teacher_logits=True):
        # the first parameter must be student and the second parameter must be teacher.
        # print(type(student_logits), type(teacher))
        # print("student_logits:", torch.var(student_logits))
        # print("teacher_logits:", torch.var(teacher))
        _student = F.log_softmax(student_logits / self.KL_temperature, dim=1)
        # _student = F.softmax(student_logits / self.KL_temperature, dim=1)
        _teacher = F.softmax(teacher / self.KL_temperature, dim=1)
        # print("student_logits:", torch.var(student_logits))
        # print("teacher_logits:", torch.var(teacher))
        divergence = F.kl_div(
            _student,
            _teacher if use_teacher_logits else teacher,
            reduction="batchmean",
        )  # forward KL
        return divergence
