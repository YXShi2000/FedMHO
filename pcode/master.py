...
class Master(object):
    ...
    def run(self):
        for comm_round in range(1, 1 + self.conf.n_comm_rounds):
            ...
            self._activate_selected_clients(
                selected_client_ids, self.conf.graph.comm_round, self.list_of_local_n_epochs
            )
    ...
    def _random_select_clients(self):
        # if "federated_average" in self.conf.fl_aggregate["scheme"]:
        #     selected_client_ids = self.conf.random_state.choice(
        #         self.client_ids, self.conf.n_participated, replace=False
        #     ).tolist()
        # else:
        #     id_interval = int(self.conf.arch_info["num_clients_per_model"])
        #     selected_client_ids = []
        #     for i in range(1, self.conf.n_clients, id_interval):
        #         selected_client_ids += self.conf.random_state.choice(
        #             self.client_ids[i:i + id_interval], self.conf.n_participated // self.num_arch, replace=False
        #         ).tolist()
        selected_client_ids = self.conf.random_state.choice(
                    self.client_ids, self.conf.n_participated, replace=False
                ).tolist()
        # selected_client_ids = random.sample(range(1, 51), 10)
    ...
    def _aggregate_model_and_evaluate(self, flatten_local_models):
        ...
        if self.aggregator.aggregate_fn is not None:
            ...
            if self.conf.fl_aggregate["scheme"] == "federated_average_ensemble":
                client_models, epoch_acc = self.aggregator.aggregate(
                    master_model=self.master_model,
                    client_models=self.client_models,
                ...
                print("\n------------- fedavg_ensemble_acc:", epoch_acc, "-------------\n")
            else:
                self.logits_remain_lst.append(self.logits_remain)
                client_models, before_clean_var, after_clean_var = self.aggregator.aggregate(
                    master_model=self.master_model,
                    client_models=self.client_models,
                    fedavg_model=fedavg_model,
                    fedavg_models=fedavg_models,
        ...
        elif self.aggregator.aggregate_fn is not None:
